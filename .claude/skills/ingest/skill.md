...
### 步驟 B2.5：數據真實性驗證 — 擠水分析（廣告報表專用）

> **核心理念**：廣告商提供的數據可能存在灌水現象。透過 GA4 與 GTM 的第一方實際數據，
> 交叉比對廣告商報告的成效指標，產出客觀的「擠水後結論」。

**觸發條件**：當 JSON 中 `ga4_page_entries`、`ga4_avg_duration`、`gtm_events` 任一欄位有值時，必須執行此步驟。若三個欄位皆為 null，則跳過此步驟並在 Source 頁面中標註「未提供 GA4/GTM 數據，無法進行擠水分析」。

#### 驗證項目一：點擊灌水檢測
比對「廣告商報告的總點擊數」（取自 date 區段 Total 的 clicks）vs「GA4 有效頁面進站數」（`ga4_page_entries`）。
```
差異率 = (廣告商點擊數 - GA4進站數) / 廣告商點擊數 × 100%
```
- 差異率 > 30%：🔴 **嚴重灌水嫌疑** — 廣告商報告的點擊有超過三成無法在 GA4 中驗證，極可能包含無效點擊或機器人流量。
- 差異率 > 15%：🟡 **中度偏差，需關注** — 存在一定比例的不可驗證流量，建議要求廣告商說明。
- 差異率 ≤ 15%：🟢 **數據可信** — GA4 與廣告商數據高度一致，屬於正常的跟蹤誤差範圍。

#### 驗證項目二：互動品質驗證
以「GA4 平均停留時間」（`ga4_avg_duration`，單位：秒）評估進站流量的真實互動品質。
- < 5 秒：🔴 **疑似無效流量** — 停留時間極短，高度懷疑為機器人或誤點擊。
- 5 ~ 15 秒：🟡 **停留時間偏低** — 使用者可能快速跳出，素材或著陸頁需最佳化。
- \> 15 秒：🟢 **互動品質良好** — 使用者有實際瀏覽行為，流量品質可信。

#### 驗證項目三：轉換真實性
比對「GTM 實際觸發事件」（`gtm_events`）與「廣告商報告的互動數」（取自 date 區段 Total 的 engagement）。
```
差異率 = (廣告商互動數 - GTM事件數) / 廣告商互動數 × 100%
```
- 差異率 > 30%：🔴 **轉換數據嚴重失真**
- 差異率 > 15%：🟡 **轉換數據有偏差**
- 差異率 ≤ 15%：🟢 **轉換數據可信**
...
#### 擠水後結論
綜合以上三項驗證結果，在 Source 頁面中產出一段「擠水後結論」，明確告知：
1. 實際可信的點擊數（以 GA4 為準）
2. 調整後的 CTR（用 GA4 進站數 / 廣告商曝光數）
3. 調整後的 CPC（用廣告花費 / GA4 進站數）
4. 整體數據可信度評級（🟢/🟡/🔴）
...
## 數據真實性驗證（擠水分析）
| 驗證項目 | 廣告商數據 | GA4/GTM 數據 | 差異率 | 評估 |
|---|---|---|---|---|
| 點擊灌水檢測 | {clicks} 次點擊 | {ga4_page_entries} 次進站 | {差異率}% | 🟢/🟡/🔴 |
| 互動品質 | - | {ga4_avg_duration} 秒 | - | 🟢/🟡/🔴 |
| 轉換真實性 | {engagement} 次互動 | {gtm_events} 次事件 | {差異率}% | 🟢/🟡/🔴 |

### 擠水後結論
- **調整後點擊數**：{ga4_page_entries}（以 GA4 為準）
- **調整後 CTR**：{ga4_page_entries / impressions × 100}%
- **調整後 CPC**：${cost / ga4_page_entries}
- **整體數據可信度**：🟢/🟡/🔴

> 若未提供 GA4/GTM 數據，此區塊應標註「⚠️ 未提供第一方驗證數據，無法進行擠水分析，建議要求廣告商補充 GA4/GTM 數據。」
